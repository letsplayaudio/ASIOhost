...
bool ProcessSamples(void *inputBuffer, void *outputBuffer, long sampleCount, int channel, ASIOSampleType bufferType)
{
	...
	for (int i = 0; i < sampleCount; i++, outBuff++, inBuff++) {
		...
		*outBuff = round(CPClipSample32* sampleOut);
	    ...
	    //	*outBuff = round(CPClipSample32* rand() / RAND_MAX);
...
{	// collect the informational data of the driver
	...
	if (ASIOGetChannels(&asioDriverInfo->inputChannels, &asioDriverInfo->outputChannels) == ASE_OK)
	{
		...
		{
			/*sprintf(tmp, "ASIOGetBufferSize (min: %d, max: %d, preferred: %d, granularity: %d);\n",
...
{	// the actual processing callback.
...
#if WINDOWS && _DEBUG && DEBUGMESSAGES
	// a few debug messages for the Windows developer
	...
	for (int i = asioDriverInfo.inputBuffers; i < asioDriverInfo.inputBuffers+asioDriverInfo.outputBuffers; i++)
	{

		if (!ProcessSamples(
			asioDriverInfo.bufferInfos[i - asioDriverInfo.inputBuffers].buffers[index],
			asioDriverInfo.bufferInfos[i].buffers[index],
			buffSize,
			i - asioDriverInfo.inputBuffers,
	...
	if (processedSamples >= asioDriverInfo.sampleRate * 20.0)	// roughly measured
